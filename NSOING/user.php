...
if(isset($_GET["user"])){
  ...
  if(isset($_POST["btnSubmit"])){
    if($user=='new'){
      ...
      $premium=False;
      $sql1 = "INSERT INTO `user`(`userName`, `password`, `email`, `birthDate`, `country`, `premium`) VALUES (?,?,?,?,?,?)";
      $preparada1= mysqli_prepare($conexion, $sql1);
      mysqli_stmt_bind_param($preparada1, "ssssb", $type,$title,$description,$height,$width,$rating,$filename,$price);
      ...
      mysqli_stmt_close($preparada1);
        if(isset($_GET["nso"])){
          ...
          $nsoPhoto=$_POST["selNsoCert"]
          $nsoIdLeage=$_POST["txtNsoPhoto"]
          $nsoCurri=$_POST["txtNsoCurri"]
          $nsoLeage=$_POST["txtNsoLeage"]

          $sql2 = "INSERT INTO `rol`(`idUser`, `rol`, `name`, `photo`, `idLeage`, `team`, `number`, `curriculum`, `idCert`) VALUES (?,?,?,?,?,?,?,?,?)";
          $preparada2= mysqli_prepare($conexion, $sql2);
          mysqli_stmt_bind_param($preparada2, "iisssiiisi", $type,$title,$description,$height,$width,$rating,$filename,$price); 
        ...
        if(isset($_GET["ref"])){
          ...
          $sql21 = "INSERT INTO `rol`(`idUser`, `rol`, `name`, `photo`, `idLeage`, `team`, `number`, `curriculum`, `idCert`) VALUES (?,?,?,?,?,?,?,?,?)";
          $preparada2= mysqli_prepare($conexion, $sql2);
          mysqli_stmt_bind_param($preparada21, "iisssiiisi", $type,$title,$description,$height,$width,$rating,$filename,$price); 
          mysqli_stmt_execute($preparada21);
          mysqli_stmt_close($preparada21);
        }
        if(isset($_GET["manager"])){
          ...
          $manLeage=$_POST["txtManagerLeage"]
           ...
           if(isset($_GET["skater"])){
          $idUser=$nuevo_id
          $manRol=4
          $=$_POST["txtSkaterName"]
            $=$_POST["txtSkaterPhoto"]
            $=$_POST["txtSkaterLeage"]
            $=$_POST["txtSkaterTeam"]
            $=$_POST["txtSkaterNumber"]

        }


      
      $sql3 = "INSERT INTO `leage`(`idLeage`, `name`, `city`, `country`, `photo`) VALUES (?,?,?,?,?)";
      $sql4 = "INSERT INTO `team`(`idTeam`, `idLeage`, `name`, `photo`) VALUES (?,?,?,?)";
      $sql5 = "INSERT INTO `certification`(`idCert`, `type`, `certification`) VALUES (?,?,?)";
      ...
      $preparada5= mysqli_prepare($conexion, $sql5);
        ...
        mysqli_stmt_bind_param($preparada3, "issss", $type,$title,$description,$height,$width,$rating,$filename,$price); 
        mysqli_stmt_bind_param($preparada4, "iiss", $type,$title,$description,$height,$width,$rating,$filename,$price); 
        mysqli_stmt_bind_param($preparada5, "iis", $type,$title,$description,$height,$width,$rating,$filename,$price);  
        ...
        ?>
        alert("se ha creado el usuario,<?=$nuevo_id?>")
        <?php
        ...
        <div class="row form-group justify-content-center">
          <div class="col-4" data-aos="fade-up" data-aos-delay="300">
            <div class="row align-items-bottom justify-content-around">
              ...
              <div class="col-7">
                <p><input type="text" id="txtUser" name="txtUser" class=" form-control" placeholder="min 8 character"maxlength="30" 
          minlength="8" 
          pattern="[0-9a-zA-ZñÑáéíóúÁÉÍÓÚ]+"
          required/></p>
